...
impl AggregateState<UpdateLocationsCommand, UpdateLocationsEvent> for UpdateLocationsState {
    ...
    fn handle_command(
        &self, command: UpdateLocationsCommand, ctx: &mut ActorContext,
    ) -> Result<Vec<UpdateLocationsEvent>, Self::Error> {
        match self {
            Self::Quiescent(state) => state.handle_command(command, ctx),
            Self::Active(state) => state.handle_command(command, ctx),
            Self::Finished(state) => state.handle_command(command, ctx),
        }
    }
    ...
    fn apply_event(
        ...
        match self {
            ...
            Self::Finished(state) => state.apply_event(event, ctx),
        }
    }
}
    ...
    fn handle_command(
        ...
        match command {
            C::UpdateLocations(zones) if !zones.is_empty() => {
                ...
                Ok(vec![UpdateLocationsEvent::Started(zones)])
            },

            C::UpdateLocations(_empty_zones) => Err(Self::Error::RejectedCommand(format!(
...
pub type LocationUpdateCompletion = BitFlags<LocationUpdateStep>;
...
impl AggregateState<UpdateLocationsCommand, UpdateLocationsEvent> for ActiveLocationsUpdate {
    ...
    fn handle_command(
        &self, command: UpdateLocationsCommand, _ctx: &mut ActorContext,
    ) -> Result<Vec<UpdateLocationsEvent>, Self::Error> {
        ...
        match command {
            C::NoteLocationObservationUpdate(zone) => {
                self.handle_location_update(zone, Step::Observation)
            },

            C::NoteLocationForecastUpdate(zone) => {
                self.handle_location_update(zone, Step::Forecast)
            },

            C::NoteLocationAlertStatusUpdate(zone) => {
                self.handle_location_update(zone, Step::Alert)
            },

            C::NoteLocationsUpdateFailure(zone) => self.handle_location_failure(zone),

            C::UpdateLocations(_) => Err(UpdateLocationsError::RejectedCommand(
...
impl ActiveLocationsUpdate {
    ...
    fn handle_location_update(
        ...
        let events = match (previous, step) {
            ...
            (Some(mut zone_steps), current) => {
                zone_steps.toggle(current);
                if zone_steps.is_all() {
                    let mut result = vec![E::LocationUpdated(
                        zone,
                        Right(UpdateCompletionStatus::Succeeded),
                    )];
                    if self.is_only_active_zone(&zone) {
    ...
    fn handle_location_failure(
        ...
        if previous_status.is_active() {
            use UpdateLocationsEvent as E;
            let mut events = vec![E::LocationUpdated(
                zone,
...
impl AggregateState<UpdateLocationsCommand, UpdateLocationsEvent> for FinishedLocationsUpdate {
    ...
    #[instrument(level = "debug", skip(self, _ctx))]
    fn apply_event(
        &mut self, event: UpdateLocationsEvent, _ctx: &mut ActorContext,
    ) -> Option<Self::State> {
        warn!(
            ?event,
